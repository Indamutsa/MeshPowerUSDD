...
def top_up_history(userinput, lang_id, language):
    ...
    if userinput == '1*' + lang_id['num'] + '*3*':
        ...
        for data in top_up:
            ...
            topup_amount = language[lang_id['lang']]['history']['top-up'] + str(data['consumed_amount']) + currency
...
def consumption_history(userinput, lang_id, language):

    #Converting the user input into a string from unicode
    userinput = str(userinput)

    if userinput == '1*' + lang_id['num'] + '*4*':
        return 'CON ' + language[lang_id['lang']]['balance']['account-balance']
        ...
        for data in consumption:
            ...
            consumed_amount = language[lang_id['lang']]['history']['consumed'] + str(data['consumed_amount']) + "frw"
            balance = language[lang_id['lang']]['history']['balance'] + str(data['cash_balance'])
